...
function showRGB4_double(Ori_HRMS,fusion,location1,location2)

th_MSrgb = image_quantile(Ori_HRMS(:,:,[3,2,1]), [0.01 0.99]);
I_fuse = image_stretch(fusion(:,:,[3,2,1]),th_MSrgb);
ent=rectangleonimage(I_fuse,location1, 0.5, 3, 1, 2.5, 1);
ent=rectangleonimage(ent,location2, 0.5, 3, 2, 2.5, 4);
figure,imshow(ent,[])
